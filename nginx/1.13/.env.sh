...
[[ ! ${name} ]] && read -e -p "Enter the name of docker container: [nginx1.13] " name
[[ ! ${name} ]] && name="nginx1.13"

# specifies the nginx host path.
volume=`sed '/^DOCKER_VOLUME=/!d;s/.*=//' ${env}`
[[ ! ${volume} ]] && read -e -p "Enter the directory path for the nginx volume: [current path] " volume
[[ ! ${volume} ]] && volume=${PWD}

# specifies the nginx external host port.
port=`sed '/^DOCKER_PORT=/!d;s/.*=//' ${env}`
[[ ! ${port} ]] && read -e -p "Enter the nginx host port: [80] " port
[[ ! ${port} ]] && port=80
